...
from src.pipeline.trainer import SequentialModelTrainer
from src.pipeline.analysis import compare_alternative_impact
from src.pipeline.visualize import create_thin_file_plots, create_comparison_plots
...
class CreditRiskPipeline:
    ...
    Methods
    -------
    preprocess(train_paths, test_paths=None):
        Run preprocessing and return datasets.
    select_models():
        Interactively select models to train.
    train_models(selected_models, datasets):
        Train selected models on all feature sets and return results.
    analyze_thin_file(datasets, all_results):
        Evaluate performance for thin-file customers.
    compare_alternative_impact(all_results, datasets):
        Compare feature set performance for thin-file customers.
    create_thin_file_plots(thin_file_results):
        Generate thin-file analysis plots.
    create_comparison_plots(df_results):
        Generate overall comparison plots.
    run(train_paths, test_paths=None):
        Execute the full sequential pipeline and return results DataFrame.
    """
    ...
    def train_models(self, trainer, selected_models, datasets):
        ...
        for idx, (model_name, model) in enumerate(selected_models.items(), 1):
            print(f"\n📈 MODEL {idx}/{len(selected_models)}: {model_name}")
    ...
    def run(self, train_paths, test_paths=None, selection: Optional[str] = None, reprocess_choice: Optional[str] = None):
        ...
        if not df_results.empty:
            ...
            print(f"\n⭐ BEST MODEL OVERALL: {best_overall['model_name']} with {best_overall['feature_set']} features")
        ...
        if thin_file_results:
            ...
            print(f"\n⭐ BEST MODEL FOR THIN-FILE: {best_thin['model_name']}")
